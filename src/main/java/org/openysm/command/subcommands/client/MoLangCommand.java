package org.openysm.command.subcommands.client;
...
public class MoLangCommand {
    ...
    public static int addWatch(CommandContext<CommandSourceStack> context, MolangWatchRegistry.EvaluationPhase watchRegistry) {
        ...
        try {
            ...
            context.getSource().sendFailure(Component.translatable("message.openysm.model.debug_animation.parser_error", e.getMessage()));
            return Command.SINGLE_SUCCESS;
        }
    }
    ...
    private static int executeExperssion(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            if (geoEntity == null) {
                geoEntity = org.openysm.capability.YSMCapabilities.get(Minecraft.getInstance().player, PlayerCapabilityProvider.PLAYER_CAP).orElse(null);
            }
            if (geoEntity != null) {
                geoEntity.executeExpression(value, true, false, str -> Minecraft.getInstance().player.sendSystemMessage(Component.translatable("message.openysm.model.debug_animation.result", str)));
            }
            return Command.SINGLE_SUCCESS;
        } catch (ParseException e) {
            context.getSource().sendFailure(Component.translatable("message.openysm.model.debug_animation.parser_error", e.getMessage()));
