...
public class AuthCommand {
    ...
    private static int addAuthModel(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (!ServerModelManager.getServerModelInfo().containsKey(string)) {
            context.getSource().sendSuccess(() -> Component.translatable("commands.openysm.export.not_exist", string), true);
            return Command.SINGLE_SUCCESS;
        }
        targets.forEach(player -> {
            org.openysm.capability.YSMCapabilities.get(player, AuthModelsCapabilityProvider.AUTH_MODELS_CAP).ifPresent(ownModelCap -> {
                ...
                context.getSource().sendSuccess(() -> Component.translatable("commands.openysm.auth_model.add.info", string, player.getScoreboardName()), true);
            });
        });
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int addAllAuthModel(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        EntityArgument.getPlayers(context, TARGETS_NAME).forEach(player -> org.openysm.capability.YSMCapabilities.get(player, AuthModelsCapabilityProvider.AUTH_MODELS_CAP).ifPresent(ownModelCap -> {
            ...
            NetworkHandler.sendToClientPlayer(new S2CSyncAuthModelsPacket(ownModelCap.getAuthModels()), player);
            context.getSource().sendSuccess(() -> Component.translatable("commands.openysm.auth_model.all.info", player.getScoreboardName()), true);
        }));
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int removeAuthModel(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        targets.forEach(player -> org.openysm.capability.YSMCapabilities.get(player, AuthModelsCapabilityProvider.AUTH_MODELS_CAP).ifPresent(ownModelsCap -> {
            ownModelsCap.removeModel(modelName);
            org.openysm.capability.YSMCapabilities.get(player, ModelInfoCapabilityProvider.MODEL_INFO_CAP).ifPresent(modelIdCap -> {
            ...
            context.getSource().sendSuccess(() -> Component.translatable("commands.openysm.auth_model.remove.info", modelName, player.getScoreboardName()), true);
        }));
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int executeClear(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        EntityArgument.getPlayers(context, TARGETS_NAME).forEach(player -> org.openysm.capability.YSMCapabilities.get(player, AuthModelsCapabilityProvider.AUTH_MODELS_CAP).ifPresent(ownModelCap -> {
            ...
            NetworkHandler.sendToClientPlayer(new S2CSyncAuthModelsPacket(ownModelCap.getAuthModels()), player);
            context.getSource().sendSuccess(() -> Component.translatable("commands.openysm.auth_model.clear.info", player.getScoreboardName()), true);
        }));
        return Command.SINGLE_SUCCESS;
    }
}