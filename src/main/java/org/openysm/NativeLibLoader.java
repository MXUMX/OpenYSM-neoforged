...
public final class NativeLibLoader {
    ...
    private static boolean loadNativeLib(String path) {
        try {
            ...
            OpenYSM.LOGGER.error("Failed to load native lib: " + path, th);
    ...
    private static byte[] readResource(String path) throws IOException {
        URL url = OpenYSM.class.getResource(path);
    ...
    private static Path ensureDirectory(Path path) {
        try {
            ...
            return FMLPaths.CONFIGDIR.get().resolve(OpenYSM.MOD_ID).resolve("cache");
        }
    }
    ...
    private static void setUnsupportedPlatformError(@Nullable String detail) {
        ...
        lastError = new ErrorState(Component.translatable("error.openysm.unsupported_platform", info), "error.openysm.unsupported_platform_ext", new Object[]{info}, "[YSM] Unsupported platform: " + info);
    }

    private static void setUnsatisfiedRuntimeError(@NotNull String msg) {
        lastError = new ErrorState(Component.translatable("error.openysm.unsatisfied_runtime_env", msg), "error.openysm.unsatisfied_runtime_env_ext", new Object[]{msg}, "[YSM] Runtime error: " + msg);
    }

    private static void setUnsatisfiedBuildError() {
        String info = SystemUtils.OS_NAME + " " + SystemUtils.OS_ARCH;
        lastError = new ErrorState(Component.translatable("error.openysm.unsatisfied_build", info), "error.openysm.unsatisfied_build_ext", new Object[]{info}, "[YSM] No build for platform: " + info);
    }
    ...
    private static void setUnsupportedLauncherError() {
        ...
        lastError = new ErrorState(Component.translatable("error.openysm.unsupported_launcher"), null, null, "[YSM] Unsupported Launcher");
    }

    private static ErrorState createLauncherError(String name, String minVer) {
        return new ErrorState(Component.translatable("error.openysm.old_launcher", name, minVer), "error.openysm.old_launcher_ext", new Object[]{name, minVer}, "[YSM] Old launcher version: " + name);
    }
