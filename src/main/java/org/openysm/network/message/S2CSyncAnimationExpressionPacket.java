...
public class S2CSyncAnimationExpressionPacket {
    ...
    public static void handleCapability(S2CSyncAnimationExpressionPacket message, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        if (context.getDirection().getReceptionSide().isClient()) {
            context.enqueueWork(() -> {
                org.openysm.capability.YSMCapabilities.get(Minecraft.getInstance().level.getEntity(message.entityId), PlayerCapabilityProvider.PLAYER_CAP).ifPresent(cap -> cap.executeAnimationExpression(message.floatData));
            });
        }
