...
public class AnimationContext<TEntity> implements IContext<TEntity> {
    ...
    private AnimationContext(TEntity entity, AnimationContext<?> context) {
        ...
        if (entity instanceof Player) {
            org.openysm.capability.YSMCapabilities.get((Player) entity, PlayerCapabilityProvider.PLAYER_CAP).ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        } else if (entity instanceof Projectile) {
            org.openysm.capability.YSMCapabilities.get((Projectile) entity, ProjectileCapabilityProvider.PROJECTILE_CAP).ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        } else if (entity instanceof Entity) {
            org.openysm.capability.YSMCapabilities.get((Entity) entity, VehicleCapabilityProvider.VEHICLE_CAP).ifPresent(cap -> this.foreignStorage = cap.getPropertyGetter());
        }
    }
