...
import org.openysm.geckolib3.extended.LivingEntityRendererAccessor;
import org.openysm.geckolib3.core.event.predicate.AnimationEvent;
import org.openysm.geckolib3.core.util.Color;
...
import org.openysm.geckolib3.util.EModelRenderCycle;
import org.openysm.geckolib3.util.IRenderCycle;
import org.openysm.mixin.client.LivingEntityAccessor;
...
public abstract class GeoReplacedEntityRenderer<TEntity extends LivingEntity, T extends LivingAnimatable<TEntity>> extends LivingEntityRenderer<TEntity, PlayerModel<TEntity>> implements IGeoRenderer<T> {
    ...
    public void renderEntityWithTexture(T t, @Nullable ResourceLocation resourceLocation, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight) {
        ...
        if (NeoForge.EVENT_BUS.post(new RenderLivingEvent.Pre(t.getEntity(), this, partialTick, poseStack, multiBufferSource, packedLight)).isCanceled()) {
            return;
        }
        ...
        if (event != null && minecraft.player != null) {
            ...
            if (t.getEntity().getVehicle() != null) {
                org.openysm.capability.YSMCapabilities.get(t.getEntity().getVehicle(), VehicleCapabilityProvider.VEHICLE_CAP).ifPresent(cap -> {
