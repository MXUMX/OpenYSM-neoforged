...
import org.openysm.client.model.ModelAssemblyFactory;
import org.openysm.client.gui.IGuiWidget;
...
import org.openysm.network.message.C2SModelSyncPayload;
import org.openysm.resource.*;
...
public class ClientModelManager {
    ...
    public static void loadDefaultModel() {
        OpenYSM.LOGGER.info("[YSM] Loading builtin default model...");
        try {
            String resourcePath = "/assets/openysm/builtin/default";
            URL resourceUrl = OpenYSM.class.getResource(resourcePath);
            if (resourceUrl == null) {
                OpenYSM.LOGGER.error("[YSM] Builtin default model not found in classpath: " + resourcePath);
                return;
            }
            ...
            ClientModelInfo  parsedBundle = YSMClientMapper.buildParsedBundle(rawModel, "default");
                ...
                OpenYSM.LOGGER.info("[YSM] Successfully pushed Default Model to render queue.");
            } catch (Exception e) {
                OpenYSM.LOGGER.error("[YSM] Failed to dispatch Default Model", e);
            }
        } catch (Exception e) {
            OpenYSM.LOGGER.error("[YSM] Failed to load builtin default model", e);
        }
    }
    ...
    private static void processServerData(ByteBuffer data) {
        ...
        try {
            ...
            OpenYSM.LOGGER.error("[YSM] Sync Error at step " + syncStep, e);
        }
    }
    ...
    private static void handlePacket01(byte[] decryptedBuffer) throws Exception {
        ...
        OpenYSM.LOGGER.info("[YSM] Exchanged Key1. Preparing to send Packet 02.");
...
//            System.out.println("Received model hash: " + mHash + ", id: " + modelId + ", unk1: " + isAuth + ", unk2: " + isCustomSkinModel);
            ...
            if (YSMClientCache.verifyFileContent(cachedFile, hash1, hash2)) {
                OpenYSM.LOGGER.info("[YSM] Cache HIT & Validated: " + ctx.uuid);
                // 命中缓存
                ...
                OpenYSM.LOGGER.info("[YSM] Cache MISS or Invalid: " + ctx.uuid + " -> Requesting...");
...
//            OpenYSM.LOGGER.info("[YSM] Cleaned up {} outdated models during sync.", modelsToRemove.size());
//        }
        ...
        if (pendingModelsCount == 0) {
            OpenYSM.LOGGER.info("[YSM] All models loaded from local cache. Handshake complete!");
            onSyncComplete();
        }
    }
    ...
    private static void handlePacket05(YSMByteBuf buf) throws Exception {
        ...
        if (ctx == null) {
            OpenYSM.LOGGER.warn("[YSM] Received unexpected file chunk for model: " + uuid);
            return;
        }
        ...
        if (ctx.bytesReceived >= totalSize) {
            ...
            OpenYSM.LOGGER.info("[YSM] Downloaded & Cached: " + outFile.getAbsolutePath());
            ...
            pendingModelsCount--;
            if (pendingModelsCount <= 0) {
                OpenYSM.LOGGER.info("[YSM] All missing models downloaded and loaded successfully!");
                onSyncComplete();
            }
        }
    }
...
//            if (true) return;
            ...
            OpenYSM.LOGGER.error("[YSM] Failed to parse and load model: " + modelId, e);
        }
    }
    ...
    private static void onSyncError(@Nullable Object obj) {
        Minecraft.getInstance().execute(() -> {
            ...
            if (obj instanceof Component component) {
                ...
                OpenYSM.LOGGER.error(component.getString(256));
            }
        });
    }
    ...
    public static void exportAllCachedModels(@Nullable String extra, @Nullable Consumer<ExportResult> callback) {
        YSMThreadPool.submit(() -> {
            try {
                ...
                for (File file : files) {
                    ...
                    try {
                        ...
                        try (YSMByteBuf outBuf = new YSMByteBuf(Unpooled.buffer())) {
                            ...
                            successCount++;
                            OpenYSM.LOGGER.info("[YSM] Successfully exported cached model to: " + exportPath);
                        }
                    } catch (Exception e) {
                        OpenYSM.LOGGER.error("[YSM] Failed to export cached model: " + file.getName(), e);
                    }
                }
                ...
                OpenYSM.LOGGER.error("[YSM] Error during batch export", e);
                if (callback != null) {
