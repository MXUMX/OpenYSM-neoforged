...
import org.openysm.client.gui.button.IconButton;
...
public class PlayerTextureScreen extends Screen {
    ...
    public void init() {
        ...
        addRenderableWidget(new FlatColorButton(this.guiLeft + 5, this.guiTop, 80, 18, Component.translatable("gui.openysm.model.return"), button -> {
        ...
        }).setTooltipText("gui.openysm.model.stop"));
        ...
        for (int animSlot = 0; animSlot < 11 && (animIndex = animSlot + (this.animationCurrentPage * 11)) < this.animationKeys.size(); animSlot++) {
            ...
            String str2 = String.format("gui.openysm.texture.button.%s", str.replaceAll("\\:", "."));
            String str3 = String.format("gui.openysm.texture.button.%s.desc", str.replaceAll("\\:", "."));
            if (I18n.exists(str2)) {
            ...
            if (I18n.exists(str3)) {
                colorButton.setTooltipLines(Lists.newArrayList(new Component[]{Component.translatable(str3).withStyle(ChatFormatting.GOLD), Component.translatable("gui.openysm.texture.button.animation_name", str).withStyle(ChatFormatting.GRAY)}));
            }
    ...
    public void renderTexturePreview(GuiGraphics guiGraphics, int scissorX, int scissorY, int scissorWidth, int scissorHeight, float partialTick) {
        ...
        org.openysm.capability.YSMCapabilities.get(this.minecraft.player, PlayerCapabilityProvider.PLAYER_CAP).ifPresent(cap -> {
