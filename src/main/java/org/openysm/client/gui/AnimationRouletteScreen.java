...
import org.openysm.client.gui.button.ConfigCheckBox;
import org.openysm.client.gui.button.FlatColorButton;
import org.openysm.client.gui.button.FlatIconButton;
...
public class AnimationRouletteScreen extends Screen {
    ...
    public void init() {
            ...
            addRenderableWidget(new FlatColorButton(this.centerX - 20, this.centerY - 10, 40, 20, Component.empty(), button -> {
                ...
                public Component getMessage() {
                    if (AnimationLockEvent.isLocked()) {
                        return Component.translatable("gui.openysm.roulette.lock_on");
                    }
                    return Component.translatable("gui.openysm.roulette.lock_off");
                }
            });
        } else {
            addRenderableWidget(new FlatColorButton(this.centerX - 20, this.centerY - 10, 40, 20, Component.translatable("gui.openysm.roulette.stop"), button2 -> {
        ...
        addRenderableWidget(new FlatColorButton(this.centerX + 125, this.centerY - 70, 145, 22, Component.translatable("gui.openysm.model.return"), button5 -> {
            navigateBack();
        }));
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        int scrolledMouseY;
        guiGraphics.drawCenteredString(this.font, Component.translatable("gui.openysm.roulette.path", StringUtils.joinWith(" > ", navigationStack.stream().map((v0) -> {
            return v0.getLeft();
    ...
    private void playAnimation(String str) {
        ...
        if (localPlayer != null && GeneralConfig.PRINT_ANIMATION_ROULETTE_MSG.get().booleanValue()) {
            localPlayer.sendSystemMessage(Component.translatable("message.openysm.model.animation_roulette.play", str));
        }
    ...
    private void navigateToSubmenu(String str) {
        if (navigationStack.size() > 5) {
            LocalPlayer localPlayer = getMinecraft().player;
            if (localPlayer != null) {
                localPlayer.sendSystemMessage(Component.translatable("gui.openysm.roulette.too_long"));
                return;
            }
            return;
        }
    ...
    private void renderKeyBindings(GuiGraphics guiGraphics, int slotIndex, int x, int y) {
        ...
        if (keyMapping.getKey() == InputConstants.UNKNOWN) {
            mutableComponentWithStyle.append(Component.translatable("key.openysm.extra_animation.none"));
        } else {
