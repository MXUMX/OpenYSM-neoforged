...
import org.openysm.client.gui.button.*;
...
import org.openysm.resource.models.ModelPackData;
...
public class PlayerModelScreen extends Screen implements IGuiWidget {
    ...
    public Screen createTextureScreen(PlayerModelScreen other, String str, ModelAssembly modelAssembly) {
        if (GeneralConfig.TEXTURE_SCREEN_MODE.get() == GeneralConfig.ScreenMode.MODERN) {
            return new ModernPlayerTextureScreen(other, str, modelAssembly);
        }
    ...
    public Screen createModelInfoScreen(PlayerModelScreen other, ModelAssembly modelAssembly) {
        if (GeneralConfig.MODEL_INFO_SCREEN_MODE.get() == GeneralConfig.ScreenMode.MODERN) {
            return new ModernModelInfoScreen(other, modelAssembly);
        }
    ...
    private void refreshModelList() {
        ...
        if (this.category == Category.AUTH) {
            org.openysm.capability.YSMCapabilities.get(localPlayer, AuthModelsCapabilityProvider.AUTH_MODELS_CAP).ifPresent(cap -> {
    ...
    public void init() {
        ...
        })).setTooltipText("gui.openysm.model.info");
        ...
        }).setTooltipText("gui.openysm.model.texture"));
        ...
        addRenderableWidget(Checkbox.builder(Component.translatable("gui.openysm.show_model_id_first"), this.font)
        ...
        }).setTooltipText("gui.openysm.all_models"));
        ...
        }).setTooltipText("gui.openysm.auth_models"));
        ...
        }).setTooltipText("gui.openysm.star_models"));
        ...
        }).setTooltipText("gui.openysm.open_model_folder.open"));
        addRenderableWidget(new FlatColorButton(this.guiLeft + 198, this.guiTop + 215, 52, 14, Component.translatable("gui.openysm.pre_page"), button10 -> {
        ...
        addRenderableWidget(new FlatColorButton(this.guiLeft + 308, this.guiTop + 215, 52, 14, Component.translatable("gui.openysm.next_page"), button11 -> {
            int currentPage = getCurrentPage();
        ...
        LazyOptional<AuthModelsCapability> capability = org.openysm.capability.YSMCapabilities.get(this.minecraft.player, AuthModelsCapabilityProvider.AUTH_MODELS_CAP);
        for (int i = 0; i < 10; i++) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.searchBox.getValue().isEmpty() && !this.searchBox.isFocused()) {
            guiGraphics.drawString(this.font, Component.translatable("gui.openysm.search").withStyle(ChatFormatting.ITALIC), this.guiLeft + 148, this.guiTop + 10, 7829367);
        }
        ...
        String strVersionString = ModList.get().getModFileById(OpenYSM.MOD_ID).versionString();
        ...
        if (this.searchBox.isHovered()) {
            MutableComponent mutableComponentWithStyle = Component.translatable("gui.openysm.search.tip").withStyle(ChatFormatting.GRAY);
            guiGraphics.pose().pushPose();
    ...
    private void renderSyncStatus(GuiGraphics guiGraphics) {
        ...
        switch (currentState.getCurrentState()) {
            case WAITING:
                mutableComponentLiteral = Component.translatable("gui.openysm.sync_hint.waiting");
                break;
            case LOADING:
                mutableComponentLiteral = Component.translatable("gui.openysm.sync_hint.loading");
                break;
            case PREPARING:
                mutableComponentLiteral = Component.translatable("gui.openysm.sync_hint.preparing");
                break;
            case SYNCING:
                if (currentState.getSyncedModels() == 0) {
                    mutableComponentLiteral = Component.translatable("gui.openysm.sync_hint.syncing");
                    break;
                } else {
    ...
    public void renderModelPreview(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (localPlayer != null) {
            ...
            try {
                InventoryScreen.renderEntityInInventoryFollowsMouse(guiGraphics, this.guiLeft + 5, this.guiTop + 19, this.guiLeft + 130, this.guiTop + 200, 70, 0.0f, (float)mouseX, (float)mouseY, localPlayer);
            } finally {
