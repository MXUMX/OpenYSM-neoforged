...
public class CuriosCompat {
    ...
    public static boolean hasItemInSlot(LivingEntity livingEntity, String str, ReferenceOpenHashSet<Item> set) {
        return IS_LOADED && CuriosApi.getCuriosInventory(livingEntity).resolve().flatMap(handler -> handler.getStacksHandler(str)).map(handler -> CuriosBinding.findInSlot(handler, itemStack -> set.isEmpty() || set.contains(itemStack.getItem())) != null).orElse(false);
    }

    public static boolean hasTaggedItemInSlot(LivingEntity livingEntity, String str, List<TagKey<Item>> list) {
        return IS_LOADED && CuriosApi.getCuriosInventory(livingEntity).resolve().flatMap(handler -> handler.getStacksHandler(str)).map(handler -> CuriosBinding.findInSlot(handler, itemStack -> {
    ...
    public static boolean hasNoTaggedItemInSlot(LivingEntity entity, String str, List<TagKey<Item>> list) {
        return IS_LOADED && CuriosApi.getCuriosInventory(entity).resolve().flatMap(handler -> handler.getStacksHandler(str)).map(handler -> CuriosBinding.findInSlot(handler, itemStack -> {
            for (TagKey<Item> itemTagKey : list) {
                if (!itemStack.is(itemTagKey)) {
                    return false;
                }
