...
import org.openysm.util.AnimatableCacheUtil;
...
public final class ModelPreviewRenderer {
    ...
    public static void renderVehicleModel(Entity entity, PoseStack poseStack, float partialTick) {
        ...
        if (vehicle != null) {
            org.openysm.capability.YSMCapabilities.get(vehicle, VehicleCapabilityProvider.VEHICLE_CAP).ifPresent(cap -> {
                int index;
                ...
                double myRidingOffset = -getPassengerRidingYOffset(vehicle, entity);
                if (((entity instanceof Player) && org.openysm.capability.YSMCapabilities.get(entity, PlayerCapabilityProvider.PLAYER_CAP).isPresent()) || TouhouLittleMaidCompat.isMaidRideable(entity)) {
                    myRidingOffset -= 0.5d;
                }
    ...
    private static void renderVehicleEntity(float yaw, Entity riderEntity, PoseStack poseStack, EntityRenderDispatcher entityRenderDispatcher, MultiBufferSource.BufferSource bufferSource, Entity vehicleEntity, float partialTick) {
        poseStack.pushPose();
        poseStack.mulPose(com.mojang.math.Axis.YP.rotationDegrees(yaw));
        entityRenderDispatcher.render(vehicleEntity, 0.0d, -getPassengerRidingYOffset(vehicleEntity, riderEntity), 0.0d, 0.0f, partialTick, poseStack, bufferSource, 15728880);
        poseStack.popPose();
    }

    private static double getPassengerRidingYOffset(Entity vehicle, Entity rider) {
        return vehicle.getPassengerRidingPosition(rider).y - vehicle.getY();
    }
