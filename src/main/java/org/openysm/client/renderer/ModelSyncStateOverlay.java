    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (syncStatus.getCurrentState() == ClientModelManager.SyncState.IDLE) {
            ...
            if (pendingModelCount > 0) {
                ...
                MutableComponent loadingText = Component.translatable("gui.openysm.sync_hint.title").append(Component.translatable("gui.openysm.sync_hint.loading_models", pendingModelCount, totalModelCount).withStyle(ChatFormatting.YELLOW));
        ...
        MutableComponent prefixText = Component.translatable("gui.openysm.sync_hint.title");

        switch (syncStatus.getCurrentState()) {
            case WAITING:
                prefixText.append(Component.translatable("gui.openysm.sync_hint.waiting").withStyle(ChatFormatting.AQUA));
                break;
            case LOADING:
                prefixText.append(Component.translatable("gui.openysm.sync_hint.loading").withStyle(ChatFormatting.GOLD));
                break;
            case PREPARING:
                prefixText.append(Component.translatable("gui.openysm.sync_hint.preparing").withStyle(ChatFormatting.LIGHT_PURPLE));
                break;
            case SYNCING:
                if (syncStatus.getSyncedModels() == 0) {
                    prefixText.append(Component.translatable("gui.openysm.sync_hint.syncing").withStyle(ChatFormatting.RED));
                } else {
