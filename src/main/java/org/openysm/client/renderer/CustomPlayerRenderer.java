...
import org.openysm.client.renderer.layer.CustomPlayerArmorLayer;
import org.openysm.client.renderer.layer.CustomPlayerElytraLayer;
import org.openysm.client.renderer.layer.CustomPlayerItemInHandLayer;
import org.openysm.client.renderer.layer.CustomPlayerParrotLayer;
...
public class CustomPlayerRenderer extends GeoReplacedEntityRenderer<Player, CustomPlayerEntity> {
    ...
    public void render(Player player, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        if (SWarfareCompat.isPlayerAiming(player) || (capability = org.openysm.capability.YSMCapabilities.get(player, PlayerCapabilityProvider.PLAYER_CAP).orElse(null)) == null) {
            return;
        }
    ...
    public ResourceLocation getTextureLocation(Player player) {
        return this.currentTexture == null ? org.openysm.capability.YSMCapabilities.get(player, PlayerCapabilityProvider.PLAYER_CAP).map((cap) -> cap.getTextureLocation()).orElse(MissingTextureAtlasSprite.getLocation()) : this.currentTexture;
    }
    ...
    public void renderNameTag(Player player, Component component, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
        ...
        if (dDistanceToSqr < 100.0d && (displayObjective = (scoreboard = player.getScoreboard()).getDisplayObjective(DisplaySlot.BELOW_NAME)) != null) {
            super.renderNameTag(player, Component.literal(Integer.toString(scoreboard.getOrCreatePlayerScore(player, displayObjective).get())).append(" ").append(displayObjective.getDisplayName()), poseStack, multiBufferSource, i, 0.0f);
            poseStack.translate(0.0d, 0.25875d, 0.0d);
        }
        super.renderNameTag(player, component, poseStack, multiBufferSource, i, 0.0f);
        poseStack.popPose();
    }
